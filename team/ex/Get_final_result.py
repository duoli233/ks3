...
def get_selfconsistency_res(prediction: list):
    ...
    prediction = [['知识库未提及'] if item == [] or item == 'None' or item == ['None'] or item == set() or item == [set()] or item == None or item == [None] or item == 'error' or item == ['error'] or item == {} else item for item in prediction]
...
if __name__ == "__main__":
    ...
    parser.add_argument('--ori_path', type=str, default="./output/test/output_result.txt")
    parser.add_argument('--output_path', type=str, default='./output/test/output.json')
